...
const workflow = createWorkflow()
	...
	// NEW: Use the generic on() method for maximum flexibility
	.on("create") // No config allowed for create event
	.on("pull_request", {
		branches: ["main"],
	})
	// Additional trigger examples using the generic on() method:
	.on("schedule", [{ cron: "0 2 * * 1" }]) // Run weekly on Mondays at 2 AM
	.on("workflow_dispatch", {}) // Allow manual triggering

	// NOTE: The specific methods still work but are deprecated:
