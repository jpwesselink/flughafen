...
type Job = Awaited<ReturnType<Octokit["rest"]["actions"]["listJobsForWorkflowRun"]>>["data"]["jobs"][number];
...
async function fetchJobLogsFromWorkflow(octokit: Octokit, job: Job): Promise<{ logs: string; job: Job }> {
...
async function getInputs(): Promise<{
	...
	if (prNumber != null) {
		...
		const existingComments = comments?.filter(
			(comment) => comment?.user?.login === "github-actions[bot]" && comment?.body?.includes(BOT_COMMENT_MARKER)
		);
...
async function getJobResults(octokit: Octokit, _token: string, sha: string): Promise<TestResultManifest> {
	...
	const integrationTestJobs = jobs?.filter((job) => /Next\.js integration test \([^)]*\) \([^)]*\)/.test(job.name));
...
function getTestSummary(sha: string, baseResults: TestResultManifest | null, jobResults: TestResultManifest) {
...
`;
	...
	const newFailedTests = currentTestFailedNames.filter((name) => !baseTestFailedNames.includes(name));
...
async function run() {
	const { token, octokit, prNumber, sha, noBaseComparison, shouldExpandResultMessages } = await getInputs();
	...
	const comments = jobResults.result.reduce((acc, value, _idx) => {
		...
		for (const testResult of testData.testResults ?? []) {
			...
			resultMessage += "\n\n";
			const failedAssertions = testResult?.assertionResults?.filter((res) => res.status === "failed");
	...
	try {
		...
		if (jobResults.result.length === 0) {
			...
			await postCommentAsync(`### Next.js test passes :green_circle: ${BOT_COMMENT_MARKER}` + `\nCommit: ${sha}\n`);
			return;
		}
