...
async function collectExamplesResult(manifestFile) {
	...
	const status = `${process.env.GITHUB_SHA}\t${timestamp}\t${passingCount}/${passingCount + failingCount}`;
...
async function collectResults(manifestFile) {
	...
	if (results.version === 2) {
		...
		const testRun = `${process.env.GITHUB_SHA}\t${timestamp}\t${passCount}/${passCount + failCount}`;
...
async function collectAndUpload(kv, { jsonPrefix, kvPrefix, deploymentDomain }) {
	...
	const developmentExamplesResult = await collectExamplesResult(`test/${jsonPrefix}dev-examples-manifest.json`);
