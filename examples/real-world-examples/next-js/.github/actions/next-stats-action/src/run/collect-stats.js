...
async function defaultGetRequiredFiles(_nextAppDir, fileName) {
	return [fileName];
}

module.exports = async function collectStats(runConfig = {}, statsConfig = {}, fromDiff = false) {
	const stats = {
		[benchTitle]: {},
	};
	const orderedStats = {
	...
	if (!fromDiff && statsConfig.appStartCommand && (hasPagesToFetch || hasPagesToBench)) {
		...
		orderedStats.General["nextStartReadyDuration (ms)"] = Date.now() - startTime;

		if (exitCode !== null) {
			throw new Error(`Failed to run \`${statsConfig.appStartCommand}\` process exited with code ${exitCode}`);
		}
