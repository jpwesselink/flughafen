...
module.exports = async function collectDiffs(filesToTrack = [], initial = false) {
	if (initial) {
		...
		await Promise.all(toRemove.map((file) => fs.rm(path.join(diffingDir, file), { recursive: true, force: true })));
	}
	const diffs = {};
		...
		filesToTrack.map(async (fileGroup) => {
			...
			await Promise.all(
				globs.map(async (pattern) => {
					curFiles.push(
						...(await glob(pattern, { cwd: statsAppDir })).filter((item) => prettierExts.includes(path.extname(item)))
					);
				})
			);
