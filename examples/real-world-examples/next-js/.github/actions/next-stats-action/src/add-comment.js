...
const round = (num, places) => {
	const placesFactor = 10 ** places;
	return Math.round(num * placesFactor) / placesFactor;
};

const shortenLabel = (itemKey) => (itemKey.length > 24 ? `${itemKey.slice(0, 12)}..${itemKey.slice(-12)}` : itemKey);
	...
	for (let i = 0; i < results.length; i++) {
		...
		Object.keys(result.mainRepoStats).forEach((groupKey) => {
			...
			itemKeys.forEach((itemKey) => {
				...
				const mainItemStr = useRawValue ? mainItemVal : prettify(mainItemVal, prettyType);

				const diffItemStr = useRawValue ? diffItemVal : prettify(diffItemVal, prettyType);
				...
				if (mainItemVal !== diffItemVal) {
					if (typeof mainItemVal === "number" && typeof diffItemVal === "number") {
						...
						if (
							...
							const warn = warnIfNegative ? (change < 0 ? "⚠️ " : "") : change > 0 ? "⚠️ " : "";
							change = `${warn}${change < 0 ? "-" : "+"}${useRawValue ? absChange : prettify(absChange, prettyType)}`;
						} else {
							change = "N/A";
						}
					} else {
				...
				if ((change !== "N/A" && !itemKey.startsWith("buildDuration")) || (isBenchmark && itemKey.match(/req\/sec/))) {
