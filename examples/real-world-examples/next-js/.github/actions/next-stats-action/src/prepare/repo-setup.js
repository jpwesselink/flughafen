...
module.exports = (actionInfo) => {
	return {
		...
		async getLastStable() {
			...
			if (!res.ok) {
				throw new Error(`Failed to get latest stable tag ${res.status}: ${await res.text()}`);
			}
		 ...
		 */
		async linkPackages({ repoDir, nextSwcVersion: nextSwcVersionSpecified, parentSpan }) {
			if (!parentSpan) {
			...
			try {
				packageFolders = await parentSpan
					...
					.traceAsyncFn(() => fs.promises.readdir(path.join(repoDir, "packages")));
			} catch (err) {
				if (err.code === "ENOENT") {
					require("node:console").log("no packages to link");
					return pkgPaths;
				}
				throw err;
			}
			...
			parentSpan.traceChild("get-pkgdatas").traceFn(() => {
				for (const packageFolder of packageFolders) {
					...
					const packedPackageTarPath = path.join(packagePath, `${packageFolder}-packed.tgz`);
					const packageJsonPath = path.join(packagePath, "package.json");

					if (!existsSync(packageJsonPath)) {
						require("node:console").log(`Skipping ${packageFolder}, no package.json`);
						continue;
					}
			...
			const nextSwcVersion = nextSwcVersionSpecified ?? pkgDatas.get("@next/swc")?.packedPackageTarPath ?? null;

			await parentSpan.traceChild("write-packagejson").traceAsyncFn(async () => {
				for (const [packageName, { packageJsonPath, packagePath, packageJson }] of pkgDatas.entries()) {
					// This loops through all items to get the packagedPkgPath of each item and add it to pkgData.dependencies
					for (const [packageName, { packedPackageTarPath }] of pkgDatas.entries()) {
						if (!packageJson.dependencies || !packageJson.dependencies[packageName]) continue;
					...
					if (packageName === "@next/swc") {
						...
						try {
							const swcBinariesDirContents = (await fs.promises.readdir(path.join(packagePath, "native"))).filter(
								(file) => file !== ".gitignore" && file !== "index.d.ts"
							);

							require("node:console").log("using swc binaries: ", swcBinariesDirContents.join(", "));
						} catch (err) {
							if (err.code === "ENOENT") {
								require("node:console").log("swc binaries dir is missing!");
							}
							throw err;
						}
			...
			await parentSpan.traceChild("pnpm-packing").traceAsyncFn(async (packingSpan) => {
				// wait to pack packages until after dependency paths have been updated
				// to the correct versions
				await Promise.all(
					Array.from(pkgDatas.entries()).map(
						async ([packageName, { packagePath: pkgPath, packedPackageTarPath: packedPkgPath }]) => {
							return packingSpan
								...
								.traceAsyncFn(async (handlePackageSpan) => {
									...
									if (packageName === "@next/swc") {
										...
										const nativeGitignorePath = path.join(pkgPath, "native/.gitignore");
										const renamedGitignorePath = path.join(pkgPath, "disabled-native-gitignore");

										await handlePackageSpan
											.traceChild("rename-gitignore")
											.traceAsyncFn(() => fs.promises.rename(nativeGitignorePath, renamedGitignorePath));
										cleanup = async () => {
											await fs.promises.rename(renamedGitignorePath, nativeGitignorePath);
										};
									}
									...
									await handlePackageSpan
										...
										.traceAsyncFn(() =>
											Promise.all([fs.promises.rename(path.join(pkgPath, packedFileName), packedPkgPath), cleanup?.()])
										);
								});
						}
					)
