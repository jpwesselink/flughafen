...
/**
 * Builder for configuring GitHub Actions with type-safe inputs
 */
export class ActionBuilder implements Builder<ActionStepConfig> {
	private config: ActionStepConfig;

	/**
	 * Create an action builder for the specified action
	 * @param actionName The action reference (e.g., "actions/checkout@v4")
	 */
	...
	build(): ActionStepConfig {
		return {
			...this.config,
			...(this.config.with
				? { with: normalizeToKebabCase(this.config.with as Record<string, string | number | boolean>) }
				: {}),
		};
	}
}
