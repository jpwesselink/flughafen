...
export class SyntaxValidator {
	...
	validate(context: ValidationContext, result: WorkflowValidationResult): void {
		try {
			...
			const hasFlughafenImport = content.includes("flughafen") || content.includes("createWorkflow");

			if (!hasImports && content.includes("createWorkflow")) {
				result.warnings.push({
					path: filePath,
					message: "Missing import statement for flughafen",
					severity: "warning",
					rule: "missing-import",
				});
			}
			...
			if (hasImports && !hasFlughafenImport && content.includes("createWorkflow")) {
				result.warnings.push({
					path: filePath,
					message: "createWorkflow used but flughafen not imported",
					severity: "warning",
