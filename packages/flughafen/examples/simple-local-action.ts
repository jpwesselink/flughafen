...
import { createLocalAction, createWorkflow } from "flughafen";
...
const workflow = createWorkflow()
	...
	.job("test", (job) =>
		job
			...
			.step((step) =>
				step.name("Setup Node.js with our custom action").uses(nodeSetupAction, (uses) =>
					uses.with({
						"node-version": "20",
						"package-manager": "npm",
					})
			...
			.step((step) => step.name("Run linting").run("npm run lint"))
	...
	.job("build", (job) =>
		job
			...
			.step((step) =>
				step.name("Setup Node.js with different version").uses(nodeSetupAction, (uses) =>
					uses.with({
						"node-version": "18",
						"package-manager": "npm",
					})
