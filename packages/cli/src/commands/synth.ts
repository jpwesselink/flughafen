...
import { processWorkflowFile, validateWorkflowFile } from "flughafen";
import type { ProcessWorkflowOptions } from "flughafen";
import type { SynthOptions } from "../types/cli";
...
export async function synthCommand(options: SynthOptions): Promise<void> {
	try {
		...
		try {
			// Use dynamic import to load flughafen module
			flughavenModule = await import("flughafen");
		} catch (_error) {
			if (verbose) {
