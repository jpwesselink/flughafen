...
import { LocalActionBuilder, createLocalAction } from "./LocalActionBuilder";
...
export class StepBuilder implements Builder<any> {
  ...
  uses<TInputs = any, TOutputs = any>(
    ...
    callback?: ((action: ActionBuilder) => ActionBuilder) | ((uses: TypedActionConfigBuilder<TInputs>) => TypedActionConfigBuilder<TInputs>)
  ): StepBuilder {
    ...
    if (action instanceof LocalActionBuilder) {
      ...
      if (callback) {
        ...
        const configuredAction = (callback as (uses: TypedActionConfigBuilder<TInputs>) => TypedActionConfigBuilder<TInputs>)(typedConfigBuilder);
  ...
  checkout(options?: Record<string, any>): StepBuilder {
    ...
    if (options?.with) {
      return this.uses('actions/checkout@v4', action => action.with(options.with));
    } else {
  ...
  setupNode(options?: Record<string, any>): StepBuilder {
    ...
    if (options?.with) {
      return this.uses('actions/setup-node@v4', action => action.with(options.with));
    } else {
...
if (import.meta.vitest) {
  ...
  describe('StepBuilder', () => {
    ...
    it('should chain multiple methods using direct form', () => {
      const step = new StepBuilder()
        .name('Complex step')
    ...
    it('should support all forms of .uses() method', () => {
      ...
      expect(step3.build().uses).toBe('./actions/demo-action');
      expect(step4.build().uses).toBe('./actions/demo-action');
  ...
  describe('Callback-based .uses() with Type Safety', () => {
    it('should support callback form for local actions with type-safe configuration', () => {
      ...
      expect(config.uses).toBe('./actions/typed-action');
      expect(config.with).toEqual({
        name: 'test-app',
        version: '1.0.0',
        enabled: true
    ...
    it('should support type-safe callback form with environment variables', () => {
      ...
      const deployAction = createLocalAction<ActionInputs>()
        ...
        .input('environment', { description: 'Target environment', required: true })
        .input('debugMode', { description: 'Enable debug mode', required: false })
        .input('timeout', { description: 'Timeout in seconds', required: false });

      const step = new StepBuilder()
        .name('Deploy application')
        .uses(deployAction, uses => uses
          .with({
            appName: 'my-app',
            environment: 'staging',
            debugMode: true,
      ...
      expect(config.uses).toBe('./actions/deploy-app');
      expect(config.with).toEqual({
        appName: 'my-app',
        environment: 'staging',
        debugMode: true,
    ...
    it('should collect local actions from callback form', () => {
      ...
      const step = new StepBuilder()
        .uses(localAction, uses => uses.with({ value: 'test' }));
    ...
    it('should support string-based input types in callback form', () => {
      ...
      const stringAction = createLocalAction<StringInputs>()
        ...
        .input('configJson', { required: true, description: 'JSON configuration as string' })
        .input('featureList', { required: true, description: 'Comma-separated feature list' })
        .input('metadataString', { required: false, description: 'Metadata as string' });

      const step = new StepBuilder()
        .name('String inputs test')
        .uses(stringAction, uses => uses
          .with({
    ...
    it('should support callback-based .uses() with complete configuration', () => {
      ...
      expect(config.uses).toBe('./actions/setup');
