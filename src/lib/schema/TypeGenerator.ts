...
export class TypeGenerator {
  ...
  generateModuleAugmentation(interfaces: GeneratedInterface[]): string {
    ...
    for (const moduleName of moduleNames) {
      ...
      lines.push('    uses<TInputs = any, TOutputs = any>(action: LocalActionBuilder<TInputs, TOutputs>): StepBuilder;');
  ...
  describe('TypeGenerator', () => {
    ...
    it('should convert action names to PascalCase interface names', () => {
      const testCases = [
        ...
        { action: 'actions/setup-node@v4', expected: 'ActionsSetupNodeV4Inputs' },
        { action: 'codecov/codecov-action@v3', expected: 'CodecovCodecovActionV3Inputs' }
      ];
