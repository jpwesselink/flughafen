...
export class TypeGenerator {
	...
	private generateInterfaceCode(
		...
		if (schema.inputs && Object.keys(schema.inputs).length > 0) {
			...
			for (let i = 0; i < inputEntries.length; i++) {
				...
				const propertyLines = this.generateProperty(
					schema.action,
					inputName,
					inputDef,
				);
	...
	private generateProperty(
		...
		const isOptional =
			!inputDef.required || (this.config.optionalDefaults && hasDefault);
	...
	generateConvenienceMethods(interfaces: GeneratedInterface[]): string {
		...
		for (const iface of interfaces) {
			...
			if (actionParts.length >= 2) {
				...
				lines.push(`   */`);
				lines.push(
					`  uses${methodName}(inputs?: ${iface.interfaceName}): StepBuilder;`,
				);
				lines.push("");
			}
		}
	...
	describe("TypeGenerator", () => {
		...
		it("should generate module augmentation from multiple schemas", () => {
			...
			const moduleAugmentation =
				generator.generateModuleAugmentation(interfaces);

			expect(moduleAugmentation).toContain(
		...
		it("should handle actions without inputs", () => {
			...
			expect(result.interfaceCode).toContain(
				"export interface EmptyActionV1Inputs {",
			);
			expect(result.interfaceCode).toContain("}");
		});
