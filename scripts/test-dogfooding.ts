...
import { ReverseApi, validate } from "../packages/core/src/index.js";
...
async function compareWorkflows(originalYaml: string, generatedYaml: string): Promise<TestResult> {
	...
	try {
		...
		// Parse both as YAML
		const originalParsed = yaml.parse(originalContent) as any;
		const generatedParsed = yaml.parse(generatedContent) as any;
